<template>
  <view class="page">
    <view class="page-header">
      <text class="page-title">骑手审核</text>
      <text class="page-tip">仅允许审核当前乡镇的骑手注册申请</text>
    </view>
    ...
    >
      ...
      <view v-else-if="applications.length === 0" class="state-wrap">
        <text class="state-text">暂无骑手申请</text>
        ...
        >
          <view class="card-top">
            <view class="rider-meta">
              <text class="rider-name">{{ item.riderName }}</text>
          ...
          <view class="info-row">
            ...
            <text class="info-value">{{ item.phone || '未提供' }}</text>
          </view>
          <view class="info-row">
            <text class="info-label">账号类型</text>
            <text class="info-value">{{ item.riderKindText }}</text>
          </view>
          <view class="info-row">
...
function normalizeApplication(item = {}) {
  ...
  return {
    ...
    lockedReason: safeText(item.audit_locked_reason || item.audit_locked_msg || item.locked_reason),
